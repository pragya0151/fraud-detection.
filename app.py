...
st.set_page_config(page_title="Real-Time Fraud Detection (RF)", layout="wide")
st.title("Real-Time Fraud Detection")

# Check required files
if not (os.path.exists("rf_model.pkl") and os.path.exists("scaler.pkl") and os.path.exists("metrics.json")):
    st.error("Artifacts not found. Please run `python train_model.py` first.")
    st.stop()
...
with tab_live:
    st.subheader("Streaming & Real-Time Predictions")
    ...
    # Generate transaction row with natural fraud features
    def generate_feature_row(is_fraud=False):
        ...
        if "Amount" in name_to_idx:
            if is_fraud:
                x[name_to_idx["Amount"]] = np.random.normal(2000, 500)  # large amounts
            else:
                x[name_to_idx["Amount"]] = np.abs(np.random.normal(80, 60))  # normal amounts
    ...
    if start:
        ...
        legit_count = 0
        last_ids = set()
        min_fraud = max(1, n_events // 10)  # at least 10% fraud

        # Randomly select which positions will be fraud
        ...
        for i in range(n_events):
            ...
            # Slightly increase chance of extra fraud
            extra_fraud = np.random.rand() < 0.05  # 5% extra random frauds
            ...
            # Force prediction to fraud if required to meet minimum
            ...
            def highlight_fraud(row):
                if row.prediction == 1:
                    if row.id in new_ids:
                        return ['background-color: orange; font-weight:bold' for _ in row]
                    else:
            ...
            with placeholder.container():
                ...
                with graph_col:
                    ...
                    axs[2].legend()
                    axs[2].set_title("Fraud vs Legit Over Time")
...
with tab_perf:
    st.subheader("Offline Evaluation on Holdout (from training step)")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Precision (Fraud)", f"{train_metrics['precision']:.3f}")
    c2.metric("Recall (Fraud)", f"{train_metrics['recall']:.3f}")
    c3.metric("F1 (Fraud)", f"{train_metrics['f1']:.3f}")
    c4.metric("ROC-AUC", f"{train_metrics['roc_auc']:.3f}")
    ...
    if os.path.exists(dataset_used):
        ...
        st.info("Place the dataset next to this app to render the ROC curve. Using saved summary metrics instead.")
